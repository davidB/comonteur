...
# fmt: off
#MISE description="Render timeline.blend's timeline to renders/ (background Blender)"
...
# dependencies = []
# ///
"""Render `timeline.blend`'s timeline.

ponytail: uses timeline.blend's own output settings and full frame range; pass
...
def main(argv: list[str]) -> int:
    exe = require_blender()
    if not Path("timeline.blend").exists():
        raise TaskError(
            f"no timeline.blend in {Path.cwd()}",
