...
| Check | Result |
|---|---|
| Link a **non-asset** Scene | **PASS.** `bpy.data.libraries.load(blend_path, link=True)` — no `assets_only=True` — links a plain generated Scene by name same as any other datablock. `library.py`'s existing `link()`/`discover()` are `assets_only=True` for `lib/` component discovery; `compositions/frames/*.blend` scenes need a separate non-asset-restricted path (`library.link_scene()`, step 3 of the plan). |
