...
One instance per video, initialized by the skill's `scripts/init` (also `mise run
comonteur:init` in a checkout; `comonteur init` in Rust at §M5.5) and operated on by the
addon/skill at runtime. `init` is
...
```
.
├── mise.toml                # pinned tools (ffmpeg, later the CLI) — written only if absent
├── mise-tasks/comonteur/    # the comonteur:* tasks: ingest-*, reconcile, edit, render, doctor
