...
| M0.11 | `blender --command extension install-file` syntax | **Confirmed via `--help`, available in 5.2**: `blender --command extension install-file -r REPO [-e] [--no-prefs] FILE`. Default local repo id is `user_default` (`blender --command extension repo-list`) — use that as `-r` in `comonteur setup` (M5.5) unless a dedicated repo is created. |
...
- `preview.py` (§6.4): call the add-on's `render_viewport_to_path` tool (or
  ...
  dir.
- M5.5 (`comonteur setup`): `-r user_default` confirmed as the install target for
